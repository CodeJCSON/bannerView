...
@interface BASegmentButton ()

@end

@implementation BASegmentButton
...
@interface BASegmentControl ()
{
    ...
    BASegmentButton *_currentBtn;   // 指示视图当前所在的按钮
...
- (void)setTitleColor:(UIColor *)titleColor {
    ...
    for (int i = 0; i < _btnCount; i++) {
        BASegmentButton *btn = self.buttons[i];
...
- (void)createUI
{
    ...
    for (int i = 0; i < _btnCount; i++)
    {
        BASegmentButton *btn = [BASegmentButton buttonWithType:UIButtonTypeSystem];
    ...
    // 第一个设置字体颜色
    _currentBtn = (BASegmentButton *)self.buttons[0];
    // 如果不设置backgroundColor，第一个按钮的文字颜色就会被设置为默认颜色
    [_currentBtn setTitleColor:DefaultCurrentBtnColor forState:UIControlStateNormal];
}
